...
-- ════════════════════════════════════════════════════════
-- CLEAN BLACKOUT LOADING SCREEN (Hides Execution Lag)
-- ════════════════════════════════════════════════════════
local function showLoadingSplash()
    ...
    backdrop.BackgroundColor3 = Color3.fromRGB(5, 5, 7)
    backdrop.BackgroundTransparency = 0
    backdrop.BorderSizePixel = 0

    local centerWrap = Instance.new("Frame", backdrop)
    centerWrap.Size = UDim2.new(0, 300, 0, 80)
    centerWrap.AnchorPoint = Vector2.new(0.5, 0.5)
    centerWrap.Position = UDim2.new(0.5, 0, 0.5, 0)
    centerWrap.BackgroundTransparency = 1

    local titleLbl = Instance.new("TextLabel", centerWrap)
    titleLbl.Size = UDim2.new(1, 0, 0, 24)
    titleLbl.Position = UDim2.new(0, 0, 0, 0)
    titleLbl.BackgroundTransparency = 1
    titleLbl.Text = "CIGANYHUB"
    titleLbl.TextColor3 = Color3.fromRGB(245, 245, 247)
    titleLbl.Font = Enum.Font.GothamBold
    titleLbl.TextSize = 18
    titleLbl.TextXAlignment = Enum.TextXAlignment.Center

    local statusLbl = Instance.new("TextLabel", centerWrap)
    statusLbl.Size = UDim2.new(1, 0, 0, 18)
    statusLbl.Position = UDim2.new(0, 0, 0, 28)
    statusLbl.BackgroundTransparency = 1
    statusLbl.Text = "Loading script..."
    statusLbl.TextColor3 = Color3.fromRGB(140, 145, 160)
    statusLbl.Font = Enum.Font.GothamMedium
    statusLbl.TextSize = 12
    statusLbl.TextXAlignment = Enum.TextXAlignment.Center

    local barBg = Instance.new("Frame", centerWrap)
    barBg.Size = UDim2.new(1, 0, 0, 3)
    barBg.Position = UDim2.new(0, 0, 0, 56)
    barBg.BackgroundColor3 = Color3.fromRGB(24, 24, 28)
    ...
    TweenService:Create(barFill, TweenInfo.new(3.0, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Size = UDim2.new(0.95, 0, 1, 0)}):Play()

    return function()
        pcall(function()
            TweenService:Create(barFill, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 1, 0)}):Play()
            task.wait(0.25)
            local fadeTween = TweenService:Create(backdrop, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1})
...
-- ════════════════════════════════════════════════════════
-- RUN SCRIPT EXECUTION (Blackout Cover During Init)
-- ════════════════════════════════════════════════════════
local function executePayload(scriptCode)
    local targetGame = getDetectedGame()
    CloseUI()

    local hideSplash = showLoadingSplash()
    ...
    task.defer(function()
        ...
        -- Hold blackout for 3.2s so the target script finishes its heavy startup, UI building, and workspace scanning without visual stutter
        task.wait(3.2)
        hideSplash()
    end)
end
